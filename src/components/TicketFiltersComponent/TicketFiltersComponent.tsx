...
const TicketFiltersComponent = ({
  ...
  const sortBy = (searchParams.get('sortBy') || initialFilters?.sortBy || 'date') as
    | 'date'
    | 'title'
    | 'priority'
    | 'dueDate';
  ...
  const updateFilters = (key: string, value: string | boolean) => {
    ...
    onFilterChange({
      ...
      sortBy: (key === 'sortBy' ? (value as string) : sortBy) as
        | 'date'
        | 'title'
        | 'priority'
        | 'dueDate',
  ...
  return (
    <div className="ticket-filters">
      ...
      <select value={priority} onChange={(e) => updateFilters('priority', e.target.value)}>
      ...
      <div className="filter-checkbox">
        ...
        />
        <label htmlFor="dueThisWeek" style={{ marginLeft: '10px' }}>
          Due this week
        </label>
      </div>
    </div>
