...
const KanbanCard = ({ ticket, onEdit, onDelete }: KanbanCardProps) => {
  ...
  return (
    ...
    >
      ...
      {ticket.description && <p className="kanban-card-description">{ticket.description}</p>}

      <div className="kanban-card-meta">
        <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
          <Badge type="priority" value={ticket.priority} />
          {ticket.dueDate && <span className="kanban-card-date">{formatDate(ticket.dueDate)}</span>}
        </div>
